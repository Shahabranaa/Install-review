...
- `artifacts/dpr` — DPR Timesheets ("DPR Timesheet — Capture & Clarify"): standalone web app for daily progress reporting. Level 1 Capture (raw manual timesheet entry: date/team/time/location/notes) + Level 2 Clarify (categorize each entry via cascading dropdowns — Activity Type → Activity Group → Activity → JDR Work Activity — with auto-populated generic comment from JDR code lookup). Manual entry only (no WhatsApp bot); Level 3/LAUTEC/claims reporting deferred. Reference data (locations, teams, activity types/groups/activities, JDR codes) seeded once from a client-provided Google Sheet. Uses the shared `users`/auth system (admin accounts only, no worker-facing login) and shares the `artifacts/api-server` backend (routes under `/api/dpr/...`) and `lib/db` schema (`dpr_*` tables). Separate product/branding from the workforce system — own domain (`dpr.spx.site`).
