...
const migrations = [
  ...
  {
    name: "dpr_teams_description",
    sql: `ALTER TABLE dpr_teams ADD COLUMN IF NOT EXISTS description text`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_teams' AND column_name = 'description'`,
  },
  {
    name: "dpr_teams_shift_times",
    sql: `ALTER TABLE dpr_teams
            ADD COLUMN IF NOT EXISTS shift_start_time text,
            ADD COLUMN IF NOT EXISTS shift_end_time text`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_teams' AND column_name = 'shift_start_time'`,
  },
  {
    name: "dpr_teams_back_team_id",
    sql: `ALTER TABLE dpr_teams ADD COLUMN IF NOT EXISTS back_team_id integer REFERENCES dpr_teams(id) ON DELETE SET NULL`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_teams' AND column_name = 'back_team_id'`,
  },
  {
  ...
  {
    name: "dpr_activity_logs",
    sql: `CREATE TABLE IF NOT EXISTS dpr_activity_logs (
      id serial PRIMARY KEY,
      actor_id integer,
    ...
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_activity_logs'`,
  },
  {
  ...
  {
    name: "dpr_custom_roles_color",
    sql: `ALTER TABLE dpr_custom_roles ADD COLUMN IF NOT EXISTS color text`,
    check: `SELECT 1 FROM information_schema.columns WHERE table_name = 'dpr_custom_roles' AND column_name = 'color'`,
  },
  {
    name: "dpr_activity_logs_entry_date",
    sql: `ALTER TABLE dpr_activity_logs ADD COLUMN IF NOT EXISTS entry_date text;`,
    check: `SELECT 1 FROM information_schema.columns WHERE table_name = 'dpr_activity_logs' AND column_name = 'entry_date'`,
  },
  {
    name: "dpr_custom_roles_table",
    sql: `CREATE TABLE IF NOT EXISTS dpr_custom_roles (
      id SERIAL PRIMARY KEY,
      abbr TEXT UNIQUE NOT NULL,
      name TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`,
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_custom_roles'`,
  },
  {
    name: "dpr_workers_roles_array",
    sql: `ALTER TABLE dpr_workers ADD COLUMN IF NOT EXISTS roles text[] NOT NULL DEFAULT '{}';
          UPDATE dpr_workers SET roles = ARRAY[role] WHERE role IS NOT NULL AND roles = '{}';`,
    check: `SELECT 1 FROM information_schema.columns WHERE table_name = 'dpr_workers' AND column_name = 'roles'`,
  },
  {
    name: "dpr_whatsapp_imports",
    sql: `CREATE TABLE IF NOT EXISTS dpr_whatsapp_imports (
      id          serial PRIMARY KEY,
      row_hash    text NOT NULL UNIQUE,
      entry_id    integer REFERENCES dpr_timesheet_entries(id) ON DELETE SET NULL,
      imported_at timestamptz NOT NULL DEFAULT now()
    )`,
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_whatsapp_imports'`,
  },
  {
    name: "dpr_team_activity_plans",
    sql: `CREATE TABLE IF NOT EXISTS dpr_team_activity_plans (
      id            serial PRIMARY KEY,
      date          text NOT NULL,
      team_id       integer NOT NULL REFERENCES dpr_teams(id) ON DELETE CASCADE,
    ...
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_team_activity_plans'`,
  },
  {
    name: "dpr_lautec_import_runs",
    sql: `CREATE TABLE IF NOT EXISTS dpr_lautec_import_runs (
      id                serial PRIMARY KEY,
      date              text NOT NULL,
      team_id           integer NOT NULL REFERENCES dpr_teams(id) ON DELETE RESTRICT,
      ...
      error_detail      text,
      requested_resend  boolean NOT NULL DEFAULT false,
      confirmed_uncertain_retry boolean NOT NULL DEFAULT false,
      actor_id          integer REFERENCES users(id) ON DELETE SET NULL,
      actor_name        text NOT NULL DEFAULT 'Unknown',
      started_at        timestamptz NOT NULL DEFAULT now(),
      finished_at       timestamptz
    )`,
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_lautec_import_runs'`,
  },
  {
    name: "dpr_lautec_import_runs_one_running_team",
    sql: `CREATE UNIQUE INDEX IF NOT EXISTS dpr_lautec_import_runs_one_running_team
          ON dpr_lautec_import_runs (date, team_id) WHERE status = 'running'`,
    check: `SELECT 1 FROM pg_indexes
            WHERE tablename = 'dpr_lautec_import_runs'
              AND indexname IN (
                'dpr_lautec_import_runs_one_running_team',
                'dpr_lautec_import_runs_one_active_team'
              )`,
  },
  {
    name: "dpr_lautec_import_runs_prevent_unconfirmed_duplicates",
    sql: `CREATE UNIQUE INDEX IF NOT EXISTS dpr_lautec_import_runs_prevent_unconfirmed_duplicates
          ON dpr_lautec_import_runs (snapshot_hash)
          WHERE status = 'success' AND requested_resend = false`,
    check: `SELECT 1 FROM pg_indexes
            WHERE tablename = 'dpr_lautec_import_runs'
              AND indexname = 'dpr_lautec_import_runs_prevent_unconfirmed_duplicates'`,
  },
  {
    name: "dpr_lautec_import_runs_confirmed_uncertain_retry",
    sql: `ALTER TABLE dpr_lautec_import_runs
          ADD COLUMN IF NOT EXISTS confirmed_uncertain_retry boolean NOT NULL DEFAULT false`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_lautec_import_runs'
              AND column_name = 'confirmed_uncertain_retry'`,
  },
  {
    name: "dpr_lautec_import_runs_one_active_team",
    sql: `DROP INDEX IF EXISTS dpr_lautec_import_runs_one_running_team;
          CREATE UNIQUE INDEX IF NOT EXISTS dpr_lautec_import_runs_one_active_team
          ON dpr_lautec_import_runs (date, team_id) WHERE status IN ('running', 'submitting')`,
    check: `SELECT 1 FROM pg_indexes
            WHERE tablename = 'dpr_lautec_import_runs'
              AND indexname = 'dpr_lautec_import_runs_one_active_team'`,
  },
  {
    name: "dpr_lautec_import_runs_mark_legacy_unverified_attempts_uncertain",
    // Earlier versions did not write a durable pre-submit checkpoint. A
    // legacy running, interrupted, or failed record cannot prove whether
    // Lautec received rows, so it must be acknowledged before retrying.
    sql: `UPDATE dpr_lautec_import_runs
          SET status = 'uncertain',
              error_detail = COALESCE(error_detail || ' ', '') ||
                'This attempt predates DPR’s submission checkpoint. Check Lautec before explicitly allowing a retry.'
          WHERE status IN ('running', 'interrupted', 'failed')`,
    check: `SELECT 1 FROM pg_constraint
            WHERE conname = 'dpr_lautec_import_runs_legacy_uncertain_migrated'`,
    after: `ALTER TABLE dpr_lautec_import_runs
            ADD CONSTRAINT dpr_lautec_import_runs_legacy_uncertain_migrated
            CHECK (true)`,
  },
  {
    name: "dpr_lautec_import_runs_remove_legacy_running_index",
    sql: `DROP INDEX IF EXISTS dpr_lautec_import_runs_one_running_team`,
    check: `SELECT 1
            WHERE EXISTS (
            ...
            AND NOT EXISTS (
              SELECT 1 FROM pg_indexes
              WHERE tablename = 'dpr_lautec_import_runs'
                AND indexname = 'dpr_lautec_import_runs_one_running_team'
            )`,
  },
  {
    name: "dpr_timesheet_entries_add_pax",
    sql: `ALTER TABLE dpr_timesheet_entries ADD COLUMN IF NOT EXISTS pax integer`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_timesheet_entries'
              AND column_name = 'pax'`,
  },
  {
    name: "dpr_add_capture_activity_dropdown_options",
    sql: `
      INSERT INTO dpr_activities (name, activity_group_id)
      SELECT options.name, options.activity_group_id
      FROM (
        VALUES
          ('QC Walkdown / As-left', 1),
          ('Standby (EDT) - Waiting on Transfer', 2),
          ('Standby (EDT) - Waiting on SAP', 2),
          ('Standby (EDT) - Waiting on Scaffolding', 2),
          ('Standby (EDT) - Rest time requirement between shifts', 2),
          ('Standby (CDT) - Rest time requirement between shifts', 2),
          ('Standby (EDT) - CTV / SOV Technical breakdown', 2),
          ('QC Walkdown / As-left', 3),
          ('Pre-Works Set up / Asset protection', 4),
          ('Fibre Optic', 4),
          ('QC Walkdown / As-left', 4),
          ('Break', 1),
          ('HSE - Toolbox Talk', 1),
          ('TBT / POWRA', 3),
          ('Welfare break on location', 3),
          ('TBT / POWRA', 4),
          ('Welfare break on location', 4)
      ) AS options(name, activity_group_id)
      WHERE NOT EXISTS (
        SELECT 1
        FROM dpr_activities activity
        WHERE activity.name = options.name
          AND activity.activity_group_id = options.activity_group_id
      )
    `,
    check: `
      SELECT 1
  ...
  {
    name: "dpr_add_missing_activity_group_pairs",
    sql: `
      INSERT INTO dpr_activities (name, activity_group_id)
      SELECT options.name, options.activity_group_id
      FROM (
        VALUES
          ('Mechanical - Hang-off', 4),
          ('Pre-Termination Testing', 4),
          ('Pre-Works Set up / Asset protection', 1),
          ('Pre-Works Set up / Asset protection', 3)
      ) AS options(name, activity_group_id)
      WHERE NOT EXISTS (
        SELECT 1
        FROM dpr_activities activity
  ...
  {
    name: "dpr_lautec_reconcile_runs_table",
    sql: `
      CREATE TABLE IF NOT EXISTS dpr_lautec_reconcile_runs (
        id serial PRIMARY KEY,
        date text NOT NULL,
    ...
    `,
    check: `SELECT 1 FROM information_schema.tables WHERE table_name = 'dpr_lautec_reconcile_runs'`,
  },
  {
    name: "dpr_lautec_reconcile_runs_one_active",
    sql: `
      CREATE UNIQUE INDEX IF NOT EXISTS dpr_lautec_reconcile_runs_one_active
      ON dpr_lautec_reconcile_runs ((true))
      WHERE status IN ('scanning', 'applying', 'saving')
    `,
    check: `SELECT 1 FROM pg_indexes WHERE indexname = 'dpr_lautec_reconcile_runs_one_active'`,
  },
  {
    name: "dpr_lautec_import_runs_rows_removed_by_reconcile_id",
    sql: `ALTER TABLE dpr_lautec_import_runs ADD COLUMN IF NOT EXISTS rows_removed_by_reconcile_id integer`,
    check: `SELECT 1 FROM information_schema.columns
            WHERE table_name = 'dpr_lautec_import_runs' AND column_name = 'rows_removed_by_reconcile_id'`,
  },
];
...
for (const migration of migrations) {
  ...
  if (rows.length > 0) {
    ...
    if (migration.after) await client.query(migration.after);
