...
test("date-tab rebuilds keep formula-looking Capture comments as literal data", () => {
  ...
  const request = buildRawSheetTabValuesRequest(
    [{ title: "2026-08-01", values: [["Working Time", "Activity", "Location", "08:00", "17:00", note, "7", "3", "VOR-008", "Routine work"]] }],
    ["Activity Group", "Activity", "Location", "Start", "Finish", "Comment", "Team ID", "PAX", "Code", "Notes"],
  );
...
test("team-scoped Lautec source reads include every managed Capture column", () => {
  assert.equal(sheetTabRange("2026-08-01", 10), "'2026-08-01'!A:J");
  assert.equal(sheetTabRange("O'Brien", 10), "'O''Brien'!A:J");
});