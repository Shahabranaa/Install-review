...
const OLD_URL = process.env.OLD_DB_URL || process.env.NEON_DATABASE_URL;
...
if (!OLD_URL) throw new Error("OLD_DB_URL or NEON_DATABASE_URL not set");
console.log("Source DB host:", OLD_URL.split("@")[1]?.split("/")[0]);
console.log("Target DB host:", NEW_URL.split("@")[1]?.split("/")[0]);
...
async function main() {
  ...
  // Neon changed default search_path to '' for security — must set explicitly
