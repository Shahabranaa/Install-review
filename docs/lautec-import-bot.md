...
The first six columns are the Lautec destination values. `Team ID` is DPR-managed source metadata: it is required to isolate the selected team's rows and is never sent to Lautec. All destination fields except Comment are required. Start and Finish must be valid `HH:MM` values. PAX comes from the Capture sheet's PAX column: it must be a whole number or blank, and a blank PAX leaves Lautec's cell untouched.
...
- Cells the import leaves empty (blank PAX, ORSTED Comments) render as `*`; verification treats `*` and blank as equally empty. When the sheet supplies a PAX, the persisted cell must read back exactly that value; any other PAX is rejected.
